...
function Nutricion() {
  ...
  const getNutrition = async () => {
    ...
    try {
      const result = await fetch(`https://api.api-ninjas.com/v1/nutrition?query=${nutrition}`, {
        method: 'GET',
        headers: {
      ...
      if (data.length === 0) {
        setError(`No se encontraron resultados para la búsqueda "${nutrition}".`);
      } else {
  ...
  return (
    <div className='font-Montserrat'>
      ...
      <div className='w-full h-[33rem] md:h-[16.81rem] bg-amber-400 relative md:block'>
        <p className='text-white w-[22rem] md:w-fit font-bold text-[1.2rem] md:text-[1.40rem] tracking-widest absolute text-center left-[7%] top-[7%] md:left-[30%] md:top-[20%]'>Toda la información nutricional que necesitas</p>
        <p className='w-[10rem] md:w-[70.5rem] text-center text-white font-[100] absolute left-[29%] top-[22%] md:left-[14%] md:top-[40%] text-[0.9rem] md:text-[1.1rem]'>
          Fitzone extrae la información nutricional de cualquier alimento del cual desees saber más.
        </p>
      ...
      <div className='w-full h-[30rem] bg-black relative'>
        <div>
          <p className='text-center text-[1rem] text-amber-400 font-[400] absolute top-[7%] left-[16%] md:top-[7%] md:left-[42%]'>Calcular información nutricional</p>
        ...
        <div className='w-[38.5rem] h-[16.187rem] absolute top-[22%] md:top-[25%] left-[15%] md:left-[30%] flex md:justify-center'>
          <div className='relative'>
            ...
            />
            <button onClick={getNutrition} className='1cursor-pointer absolute right-0 top-[-7%] left-[85%] bottom-0 mt-4 mr-4 bg-zinc-800 z-10 w-[2.9rem] h-[3.1rem] rounded-full'>
        ...
        {nutritionData && nutritionData.length > 0 ? (
          <div className='w-[19rem] md:w-[31rem] md:h-[14rem] h-[15rem] bg-zinc-700 rounded-xl absolute top-[40%] left-[12%] md:left-[34%]'>
            {nutritionData.map((item, index) => (
              <div key={index} className='w-[15rem] h-[10rem] md:ml-[8rem] ml-[2rem]'>
                <p className='text-white text-[1.4rem] md:text-[2rem] font-[700] uppercase mt-5 ml-[5.5rem] md:ml-[5rem]'>{item.name}</p>
                ...
                <div className='w-[15rem] h-[2rem] flex justify-between'>
                  ...
                  <p className='text-amber-400'>{item.carbohydrates_total_g} g</p>
                </div>
              </div>
            ))}
          </div>
